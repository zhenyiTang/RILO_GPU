...
def torch_RMSE(x):
    zeros_tensor = torch.zeros([x.shape[0], x.shape[1]], dtype=torch.float32)
    return torch.sqrt(torch.mean((x - zeros_tensor)**2))

def rms(x):
    return sqrt(mean_squared_error(x, np.zeros(x.shape[1])))
...
class DS_Policy_3(Dataset):
    def __init__(self, traj):
        self.dat = []
        for dat in traj:
            obs, act, ob2 = dat
            self.dat.append([obs, act, ob2])

    def __len__(self):
        return len(self.dat)

    def __getitem__(self, idx):
        obs, act, ob2 = self.dat[idx]
        return obs, act, ob2
